		...
		for (int c = 0; c < 2; c++) {
			...
			if ((arrayLineasCaratulas.get(0).get(c)).trim().length() == 0
				...
				System.out.println("Variable " + c
						+ " de la CARATULA se encuentra sin informaci�n");
			}
		}

//		if (arrayLineasCaratulas.get(0).get(0).length() < 3 || arrayLineasCaratulas.get(0).get(0).length() > 10) {
//			System.err.println("Rut Emisor Libro no cumple con tama�os Min 3, Max 10");
//		}
		if (arrayLineasCaratulas.get(0).get(1).length() < 3
				|| arrayLineasCaratulas.get(0).get(1).length() > 10) {

			System.err.println("Rut Envia no cumple con tama�os Min 3, Max 10");

		}
		if (arrayLineasCaratulas.get(0).get(2).length() != 7) {
			System.err.println("Tama�o fecha Pediodo Tributario incorrecto");
		}

		this.validarFechaAAAAMM(arrayLineasCaratulas.get(0).get(2));

		if (arrayLineasCaratulas.get(0).get(3).length() != 10) {
			System.err.println("Tama�o fecha Resoluci�n incorrecto");
	...
	/**
	 * Transforma si datos vienen invertidos en formato DD MM AAAA y da como resultado AAA-MM-DD 
	 * @param date
	 * @return
	 */
	...
	public void validaEntradaLCVDetalles(ArrayList<ArrayList<String>> arrayLineasDetalles,int cantidadDetalles) {
		...
		if(arrayLineasDetalles.get(x).get(0).trim().length() == 0 || arrayLineasDetalles.get(x).get(0) == null){
			System.err.println("Detalles Tipo Doc file ("+x+",0) est� vac�o o nulo");
		}
		if(arrayLineasDetalles.get(x).get(3).trim().length() == 0 || arrayLineasDetalles.get(x).get(3) == null){
			System.err.println("Detalle Numero Documento fila ("+x+",3) est� vac�o o nulo");
		}}
	}
	/**
	 * @param fecha Valida formato de Fecha A�O-MES
	 */
	...
	/**
	 * @param fecha Valida formato de Fecha A�O-MES-DIA
	 */
    ...
    /**
     * Transforma fecha YYMMDDToDDMMYY, valida / a -
     * @param fechaS
     * @return
     */
    ...
    public void validaEntradaDetalles(ArrayList<ArrayList<String>> arrayLineasDetalle,int cantidadDetalles){
    	...
    	for(int x=0; x<cantidadDetalles;x++){
			...
			if(arrayLineasDetalle.get(x).get(0).trim().length() == 0 || arrayLineasDetalle.get(x).get(0) == null){
				System.err.println("Detalle NroLinDet ("+x+",0) est� vac�o o nulo");
			}
			if(arrayLineasDetalle.get(x).get(3).trim().length() == 0 || arrayLineasDetalle.get(x).get(3) == null){
				System.err.println("Detalle NmbItem ("+x+",3) est� vac�o o nulo");
			}
			if(arrayLineasDetalle.get(x).get(12).trim().length() == 0 || arrayLineasDetalle.get(x).get(12) == null){
				System.err.println("Detalle MontoItem ("+x+",12) est� vac�o o nulo");
			}
    		}
    	}
    ...
    public void validaEntradaDscRcgGlobal(String[][] arrayLineasDscRGlobal,int cantidadDscRGlobal){
    	...
    	for(int x=0; x<cantidadDscRGlobal;x++){
			...
			if(arrayLineasDscRGlobal[x][0].trim().length() == 0 || arrayLineasDscRGlobal[x][0] == null){
				System.err.println("Descuento Recarga NroLinDR("+x+",0) est� vac�o o nulo");
			}
			if(arrayLineasDscRGlobal[x][1].trim().length() == 0 || arrayLineasDscRGlobal[x][1] == null){
				System.err.println("Descuento Recarga TpoMov ("+x+",1) est� vac�o o nulo");
			}
			if(arrayLineasDscRGlobal[x][3].trim().length() == 0 || arrayLineasDscRGlobal[x][3] == null){
				System.err.println("Descuento Recarga TpoValor ("+x+",3) est� vac�o o nulo");
			}
			if(arrayLineasDscRGlobal[x][4].trim().length() == 0 || arrayLineasDscRGlobal[x][4] == null){
				System.err.println("Descuento Recarga ValorDR ("+x+",4) est� vac�o o nulo");
			}
    		}
    	}
    public String validaRut(String rut){
		if (!rut.contains("-")){
			if (rut.contains("K") || rut.contains("k")){
				...
				rut=rut.replaceAll("K", "-K");
				rut=rut.replaceAll("k", "-k");
				return rut;
			}else{
				rut=rut.replaceAll("[^\\dA-Za-z ]", "");
				String aux1=rut.substring(0, rut.length()-1);
				String aux2=rut.substring(rut.length()-1, rut.length());
				return aux1+"-"+aux2;
			}
		}
		return rut;
    ...
    /**
     * Valida decimales con 2 cifras
     * @param valor
     * @return
     */
    public Double validaDecimales(Double valor){    	
    	DecimalFormat df = new DecimalFormat("#.00");		     	    	
    	valor = Double.parseDouble(df.format(valor).replaceAll(",","."));
